...
switch ( $action ) {
	...
	case 'confirm_admin_email':
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'moco-anmeldung.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			<?php
			/**
	...
	case 'retrievepassword':
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'moco-anmeldung.php?checkemail=confirm';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
		...
		<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( 'moco-anmeldung.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
			<p>
	...
	case 'rp':
		...
		if ( ! $user || is_wp_error( $user ) ) {
			...
			if ( $user && $user->get_error_code() === 'expired_key' ) {
				wp_redirect( site_url( 'moco-anmeldung.php?action=lostpassword&error=expiredkey' ) );
			} else {
				wp_redirect( site_url( 'moco-anmeldung.php?action=lostpassword&error=invalidkey' ) );
			}
		...
		?>
		<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( network_site_url( 'moco-anmeldung.php?action=resetpass', 'login_post' ) ); ?>" method="post" autocomplete="off">
	...
	case 'register':
		...
		if ( ! get_option( 'users_can_register' ) ) {
			wp_redirect( site_url( 'moco-anmeldung.php?registration=disabled' ) );
			exit;
		}
		...
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : 'moco-anmeldung.php?checkemail=registered';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
		...
		?>
		<form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'moco-anmeldung.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
			<p>
		...
		<p id="nav">
			...
			/** This filter is documented in moco-anmeldung.php */
	...
	case 'checkemail':
		...
		/** This action is documented in moco-anmeldung.php */
	...
	default:
		...
		<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'moco-anmeldung.php', 'login_post' ) ); ?>" method="post">
			<p>
				<label for="user_login"><?php _e( 'Username or Email Address' ); ?></label>
